...
function getToken() {
  return localStorage.getItem('sdpms_access')
}

function setTokens(access, refresh) {
  localStorage.setItem('sdpms_access', access)
  localStorage.setItem('sdpms_refresh', refresh)
}

function clearTokens() {
  localStorage.removeItem('sdpms_access')
  localStorage.removeItem('sdpms_refresh')
  ...
  if (res.status === 401) {
    ...
    if (refreshed) {
      ...
      if (!retry.ok) {
        ...
        throw new Error(errorData.detail || retry.statusText)
      }
...
export function getSession() {
  try {
    return JSON.parse(localStorage.getItem('sdpms_user') || '{}')
  } catch {
    return {}
  }
}
...
export async function updateMe(payload) {
  const user = await request('/auth/me/update/', {
    method: 'PATCH',
    body: JSON.stringify(payload),
  })
  localStorage.setItem('sdpms_user', JSON.stringify(user))
...
export async function softDeleteProject(id) {
  return request(`/projects/${id}/?action=soft`, { method: 'DELETE' })
}

export async function restoreProject(id) {
  return request(`/projects/${id}/?action=restore`, { method: 'DELETE' })
}

export async function permanentDeleteProject(id) {
  return request(`/projects/${id}/?action=permanent`, { method: 'DELETE' })
}

export async function incrementProjectViews(id) {
  return request(`/projects/${id}/views/`, { method: 'POST' })
}
...
export async function getTopProjects() {
  return request('/discover/top/')
}

export async function getPortfolioDesign() {
  return request('/portfolio/')
}

export async function savePortfolioDesign(payload) {
  return request('/portfolio/', {
    method: 'POST',
    body: JSON.stringify(payload),
  })
...
export async function incrementPortfolioView(userId) {
  return request(`/profile/${userId}/view/`, { method: 'POST' })
}
...
export async function getTrashedTemplates() {
  return request('/templates/trashed/')
}

export async function createTemplate(payload) {
  return request('/templates/create/', {
    method: 'POST',
    body: JSON.stringify(payload),
  })
}
...
export async function deleteTemplate(id, action = 'soft') {
  return request(`/templates/${id}/?action=${action}`, { method: 'DELETE' })
}
...
export async function adminToggleSuspend(id) {
...
export async function resolveFlag(id, action) {
  return request(`/moderation/resolve/${id}/`, {
    method: 'POST',
